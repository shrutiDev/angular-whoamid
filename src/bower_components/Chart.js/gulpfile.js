...
gulp.task('build', function () {
  ...
  var srcFiles = [FileName('Core')], isCustom = !!util.env.types, outputDir = isCustom ? 'custom' : '.';
  if (isCustom) {
...
gulp.task('bump', function (complete) {
  ...
  inquirer.prompt({
    ...
    var increment = res.version.split(' ')[0], newVersion = semver.inc(package.version, increment);
